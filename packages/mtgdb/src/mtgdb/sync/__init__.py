...
Usage:
    from mtgdb.sync import sync_all
    sync_all()  # Full sync
...
def sync_all(force: bool = False) -> bool:
    ...
    _section("Downloading MTGJSON sources")
    ...
    # Sync tables
    _section("Syncing reference tables")
    ...
    console.print(f"\n[bold green]✓ Sync complete.[/bold green] Database: [dim]{DB_PATH}[/dim]")
    return True
...
def _compute_word_frequencies():
    ...
    with Session(engine) as session:
        total_cards = session.exec(select(func.count()).select_from(MJCard)).one()
        with rows_progress() as progress:
            task = progress.add_task("Word frequencies", total=total_cards)
    ...
    # Save frequencies
    output_path = MTGDB_HOME / "corpus_wordfreq.json"
    ...
    console.print(f"  [green]✓[/green] Word frequencies: {len(word_counts):,} unique tokens")
