...
class LayerBoundaryValidator:
    ...
    def __init__(self):
        ...
        # Pydantic and SQLModel are acceptable in domain models since SQLModel extends Pydantic
        # The key is avoiding FastAPI-specific logic, not Pydantic itself
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ImportFrom):
                        if node.module and "mtgsim.api.models" in node.module:
                            self.violations.append(f"❌ {file_path}: Data access layer imports API models directly")
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom):
                    if node.module and "mtgsim.db.domain_models" in node.module:
                        self.violations.append(f"❌ {file_path}: API model imports domain models directly")
        ...
        try:
            ...
            for func_name in required_functions:
                if func_name not in content:
                    self.violations.append(f"❌ Missing converter function: {func_name}")
            ...
            if file_path.exists():
                try:
                    ...
                    if "mtgsim.api" in content:
                        dependency_violations.append(f"❌ {file_path}: Domain layer depends on API layer")

                    if "mtgsim.api.data" in content:
                        dependency_violations.append(f"❌ {file_path}: Domain layer depends on data access layer")
