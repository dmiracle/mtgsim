    ...
    **Validates: Requirements 5.4**
    ...
    with temp_domain_db() as db_path:
        with get_domain_session(db_path) as session:
            ...
            test_card = create_test_card(session, card_uuid, card_name, set_code)
            ...
            assert len(cards_after_rollback) == len(initial_card_count), "Card count should be restored to initial state"
    ...
    **Validates: Requirements 5.4**
    ...
    with temp_domain_db() as db_path:
        with get_domain_session(db_path) as session:
            ...
            # Add set and cards
            test_set = create_test_set(session, set_code, set_name)
            added_card_uuids = []
    ...
    **Validates: Requirements 5.4**
    ...
    with temp_domain_db() as db_path:
        with get_domain_session(db_path) as session:
            ...
            initial_card = create_test_card(session, "initial-card-uuid", "Initial Card", "INI")
            initial_card_count = len(session.exec(select(DomainCard)).all())
            ...
            new_card = create_test_card(session, "new-card-uuid", "New Card", "NEW")
...
@given(
    migration_already_rolled_back=st.booleans(),
    can_rollback_flag=st.booleans()
)
@settings(max_examples=10, suppress_health_check=[HealthCheck.function_scoped_fixture])
    ...
    with temp_domain_db() as db_path:
        with get_domain_session(db_path) as session:
            ...
            if not migration_already_rolled_back:
                test_card = create_test_card(session, "test-card-uuid", "Test Card", "TST")
...
@given(
    keep_days=st.integers(min_value=1, max_value=30)
)
@settings(max_examples=10, suppress_health_check=[HealthCheck.function_scoped_fixture])
