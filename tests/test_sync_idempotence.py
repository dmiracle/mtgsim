...
class TestSyncOperationIdempotence:
    ...
    @given(
        table_name=st.text(min_size=1, max_size=15, alphabet=st.characters(whitelist_categories=("Ll", "Lu"))).filter(
            lambda x: x.isidentifier() and not x.startswith("_") and x.lower() not in SQL_RESERVED
        ),
    ...
    @given(
        schema_variations=st.lists(
            st.text(min_size=1, max_size=10, alphabet=st.characters(whitelist_categories=("Ll", "Lu"))).filter(
                lambda x: x.isidentifier() and x.lower() not in SQL_RESERVED
            ),
            min_size=1,
            max_size=5,
