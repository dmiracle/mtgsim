...
class TestLayerBoundaryEnforcement:
        ...
        **Feature: domain-api-consolidation, Property 13: Layer Boundary Enforcement**
        ...
        for domain_file in self.domain_files:
            if domain_file.exists():
                ...
                if upward_deps:
                    violations.extend([
                        f"{domain_file}: depends on {dep}" for dep in upward_deps
                    ])
    ...
    @given(st.lists(
        st.sampled_from([
            "fastapi", "starlette", "uvicorn", "httpx", "requests", 
    ...
    def test_overall_layer_boundary_validation_passes(self):
        ...
        **Feature: domain-api-consolidation, Property 13: Layer Boundary Enforcement**
        ...
        assert len(violations) == 0, (
            f"Found {len(violations)} layer boundary violations: {violations}"
        )
    ...
    def _get_presentation_logic_violations(self, file_path: Path) -> List[str]:
        ...
        try:
            ...
            presentation_keywords = {
                "serialize", "render", "template", "response", "request", 
                "endpoint", "route", "json_response", "format_for_api"
            }
